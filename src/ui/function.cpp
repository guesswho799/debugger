      ...
      ftxui::Renderer(
          ...
          [&] {
            ...
            if (get<AppState>()->in_search_mode) {
              table.SelectRow(_state.function_selector)
                  .Decorate(ftxui::bgcolor(ftxui::Color::Grey11));
              return ftxui::vbox(
                         {ftxui::window(ftxui::text("Search function"),
                                        _state.function_input->Render()),
                          ftxui::separator(),
                          ftxui::window(ftxui::text("Functions"),
                                        table.Render() | ftxui::frame |
                                            ftxui::flex)}) |
                     ftxui::flex | ftxui::border;
            } else {
              auto content = table.SelectRows(1, -1);
              content.DecorateCellsAlternateRow(
                  ftxui::bgcolor(ftxui::Color::Default), 2, 0);
              content.DecorateCellsAlternateRow(
                  ftxui::bgcolor(ftxui::Color::Grey11), 2, 1);
              return ftxui::vbox(
                         {ftxui::text(" Press / to start searching..."),
                          ftxui::separator(),
                          ftxui::window(
                              ftxui::text("Functions"),
                              table.Render() |
                                  ftxui::focusPositionRelative(
                                      _state.scroll_x, _state.scroll_y) |
        ...
        if (get<AppState>()->in_search_mode) {
          ...
          if (event == ftxui::Event::Return) {
            const auto function_name =
                _state.function_table_content[_state.function_selector][0];
