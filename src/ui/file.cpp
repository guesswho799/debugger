...
ftxui::Component File::get_logic()
{
    return _logic;
}
...
{
    return ftxui::Renderer(_state.file_input, [&]
    {
        _state.files.clear();
        ...
        {
            const size_t last_directory_index = _state.file_path.find_last_of('/');
            if (last_directory_index == std::string::npos) { search_path = "."; }
            else { search_path = std::string(_state.file_path.c_str(), last_directory_index); }
        }
        if (std::filesystem::is_directory(search_path))
        {
            ...
            {
                _state.files.emplace_back(ftxui::MenuEntry(file.path().string()));
        ...
        auto menu = ftxui::Container::Vertical(_state.files, &_state.open_file_selector);
        if (get<AppState>()->in_search_mode)
        {
            return ftxui::vbox({
                ftxui::window(ftxui::text("Search file"), _state.file_input->Render()),
                ftxui::separator(),
    	    ...
    	    if (event == ftxui::Event::Escape)
	    {
    	    ...
    	    if (get<AppState>()->in_search_mode)
    	    {
                if (event == ftxui::Event::ArrowUp or event == ftxui::Event::TabReverse) _state.open_file_selector--;
                if (event == ftxui::Event::ArrowDown or event == ftxui::Event::Tab) _state.open_file_selector++;
                if (_state.open_file_selector < 0)                               _state.open_file_selector = 0;
		const auto files_size = _state.files.end() - _state.files.begin() - 1;
                if (_state.open_file_selector > files_size) _state.open_file_selector = files_size;
                if (event == ftxui::Event::Return)
                {
		    get<AppState>()->update_binary(_state.files_as_strings[_state.open_file_selector]);
