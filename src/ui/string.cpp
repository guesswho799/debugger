...
ftxui::Component String::get_logic()
{
    return _logic;
}
...
{
    // TODO: fix slider acting wierd, probably need to add main component to renderer
    auto string_tab = ftxui::Renderer([&]
    {
        return Loader::load_strings(get<AppState>()->static_debugger.value()) | ftxui::focusPositionRelative(_state.scroll_x, _state.scroll_y) | ftxui::vscroll_indicator | ftxui::frame | ftxui::flex;
    });
    return ftxui::Container::Horizontal({string_tab, _state.scrollbar_y}) | ftxui::flex;
}
