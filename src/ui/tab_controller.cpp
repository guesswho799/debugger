...
ftxui::Component TabController::get_logic()
{
    return _logic;
}
...
TabController::State_t TabController::_generate_initial_state()
{
    ...
    return {
        .tab_values     = {"Display", "Execute"},
	.display_values = {"Main", "Open file", "Functions", "Strings"},
	.execute_values = {"Information", "All functions", "Single function"},
        .tab_toggle     = ftxui::Menu(&_state.tab_values, &get<AppState>()->tab_selected, toggle_option),
	.display_toggle = ftxui::Menu(&_state.display_values, &get<AppState>()->display_selected, tab_option),
        .execute_toggle = ftxui::Menu(&_state.execute_values, &get<AppState>()->execute_selected, tab_option),
        .display_main   = ftxui::Container::Tab({get<Code>()->get_logic(), get<File>()->get_logic(), get<Function>()->get_logic(), get<String>()->get_logic()}, &get<AppState>()->display_selected),
        .execute_main   = ftxui::Container::Tab({Info::get_logic(), get<MultiTrace>()->get_logic(), get<SingleTrace>()->get_logic()}, &get<AppState>()->execute_selected),
        .tab_container  = ftxui::Container::Tab({ ftxui::Renderer([&]
   	{
            return ftxui::vbox({
	        ftxui::hbox({
	            ftxui::text(" "),
		    _state.display_toggle->Render()
...
{
    return ftxui::Renderer(_state.display_main, [&]
    {
        return ftxui::vbox({
            ftxui::hbox({_state.tab_toggle->Render() | ftxui::flex, ftxui::text("Quit")}),
    	    _state.tab_container->Render()
        });
    }) | ftxui::CatchEvent([&](ftxui::Event event)
    {
    	if (get<AppState>()->in_search_mode) return false;
    	...
    	else if (get<AppState>()->tab_selected == 0 and event == ftxui::Event::Character('s'))
    	{
    	...
    	else if (get<AppState>()->tab_selected == 1 and event == ftxui::Event::Character('s'))
    	{
    	...
    	if (get<AppState>()->tab_selected == 1 and get<AppState>()->execute_selected == 2 and event == ftxui::Event::Character('v'))
    	{
    	...
    	if (get<AppState>()->tab_selected == 1 and get<AppState>()->execute_selected == 2)
    	{
    	    if (event == ftxui::Event::ArrowLeft)
    	    {
    		if (get<SingleTrace>()->get_selector() != 0) get<SingleTrace>()->dec_selector();
    		return true;
    	    }
    	...
    	if ((get<AppState>()->tab_selected == 1 and get<AppState>()->execute_selected == 2) or (get<AppState>()->tab_selected == 0 and get<AppState>()->display_selected == 0))
    	{
    	    if (event == ftxui::Event::ArrowDown)
    	    {
    		uint64_t code_size = 0;
    		for (const auto& line : get<AppState>()->static_debugger.value().get_function_code_by_name(get<AppState>()->function_name))
    		{
    		...
    		const uint64_t main_display_size = (get<AppState>()->tab_selected == 1 and get<AppState>()->execute_selected == 2) ? screen_size : screen_size + 4;
    		if (code_size - get<Code>()->get_selector() >= main_display_size) get<Code>()->inc_selector();
    	        get<Code>()->update_code();
    		return true;
    	    }
    	    else if (event == ftxui::Event::ArrowUp)
    	    {
    		if (get<Code>()->get_selector() != 0) get<Code>()->dec_selector();
    	        get<Code>()->update_code();
    		return true;
    	    }
    	}
