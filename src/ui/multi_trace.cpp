...
ftxui::Component MultiTrace::get_logic()
{
    return _logic;
}
...
ftxui::Component MultiTrace::_generate_logic()
{
    ...
    { 
	    ...
	    title = "Running process...";
            const auto functions = get<AppState>()->static_debugger.value().get_implemented_functions();
	...
	else
	{
        ...
        const auto runtime_value = get<AppState>()->dynamic_debugger.value().get_runtime_arguments();
        return ftxui::vbox({
	    ftxui::text(title),
	    ftxui::separator(),
