...
ElfReader::ElfReader(std::string file_name)
    : file(std::ifstream(file_name)), _file_name(file_name),
      header(header_factory()), sections(sections_factory()),
...
std::vector<NamedSection> ElfReader::sections_factory() {
  ...
  for (const auto &section : sections) {
    file.seekg(sections[header.section_table_name_index].unloaded_offset +
               section.name_offset);
...
std::vector<NamedSymbol> ElfReader::static_symbols_factory() {
  ...
  ElfSymbol symbol{};
  while (static_cast<uint64_t>(file.tellg()) <
...
ElfReader::get_function_code(NamedSymbol function) const {
  ...
  int offset = function.value -
               sections[function.section_index].loaded_virtual_address +
               sections[function.section_index].unloaded_offset;
  file.seekg(offset);
