...
namespace Ptrace
{
	...
	errno = 0;
        if (ptrace(PTRACE_GETREGS, pid, NULL, &regs) == -1 or errno != 0) { throw CriticalException(Status::ptrace__peek_regs_failed); }
	return regs;
    }
    ...
    inline uint64_t get_code(pid_t pid, uint64_t address)
    {
        ...
        if (errno != 0) { throw CriticalException(Status::ptrace__peek_code_failed); }
	return code;
    }
    ...
    {
        errno = 0;
        if (ptrace(PTRACE_SINGLESTEP, pid, NULL, NULL) == -1 or errno != 0) { throw CriticalException(Status::elf_runner__step_failed); }
    }

    inline void cont(pid_t pid)
    {
	errno = 0;
        if (ptrace(PTRACE_CONT, pid, NULL, NULL) == -1 or errno != 0) { throw CriticalException(Status::ptrace__cont_failed); }
    }
}
