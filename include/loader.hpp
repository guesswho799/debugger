...
load_instructions(const std::string &function_name,
  ...
  const int main_display_height =
      line_selected.has_value() ? screen_height - 12 : screen_height - 6;
  ...
  for (const auto &line : assembly | std::views::drop(scroll_bar)) {
    ...
    const int width = address.str().length() +
                      std::min(amount_of_opcodes, opcodes_per_line) * 2 + 2 +
                      line.instruction.length() + 1 + line.arguments.length() +
                      4;
    ...
    auto content =
        ftxui::hbox({ftxui::text(address.str()), ftxui::separator(),
                     get_opcodes(line), ftxui::separator(),
  ...
  return block | ftxui::border |
         ftxui::size(ftxui::WIDTH, ftxui::EQUAL, max_line_width) |
         ftxui::center;
}
